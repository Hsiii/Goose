import type { GooseDirection, GooseMotionBounds, GooseMotionSnapshot } from "$lib/types";
...
function nextIdleState(snapshot: GooseMotionSnapshot, bounds: GooseMotionBounds): GooseMotionSnapshot {
  return {
    ...snapshot,
...
function nextInspectState(snapshot: GooseMotionSnapshot, bounds: GooseMotionBounds): GooseMotionSnapshot {
  return {
    ...snapshot,
    x: clamp(snapshot.x, SIDE_PADDING, bounds.width - SIDE_PADDING),
    y: bounds.floorY,
...
function nextWalkState(snapshot: GooseMotionSnapshot, bounds: GooseMotionBounds): GooseMotionSnapshot {
...
export function createInitialGooseMotion(bounds: GooseMotionBounds): GooseMotionSnapshot {
...
export function tickGooseMotion(
  ...
  let nextSnapshot: GooseMotionSnapshot = {
    ...
    gaitPhase:
      snapshot.gaitPhase + deltaMs * (snapshot.state === "walk" ? 0.013 : snapshot.state === "inspect" ? 0.0045 : 0.0028)
  };
  ...
  if (nextSnapshot.state === "walk") {
    ...
    const travel = Math.min(Math.abs(distance), nextSnapshot.speed * deltaSeconds);
    ...
    if (Math.abs(nextSnapshot.targetX - nextSnapshot.x) <= 2 || nextSnapshot.stateTimerMs >= nextSnapshot.stateDurationMs) {
